...
int main(){
    ...
    cout << "Which simulation mode do you want? (analog / implicit) ";
    cin >> mode_input;
    cout << endl;

    if (mode_input == "analog") {
        chosen_mode = Mode::analog;}
    else if (mode_input == "implicit") {
        chosen_mode = Mode::implicit_capture;}
         else {
            std::cerr << "Error:' the mode '" << mode_input << "' does not exist (use 'analog' or 'implicit')" << std::endl;
            std::exit(1);}


    std::string material_name;
    cout << "What material will you use? ";
    cin >> material_name;
    cout << endl;
    ...
    cout << "How many particles will there be in this simulation? " ;
    cin >> N_particles;
    cout << endl;
    
    cout << "What do you want to be the max number of collisions befor the end of the simulation of a particle? " ;
    cin >> max_collisions;
    cout << endl;

    double inner_radius, outer_radius, height;
    
    cout << "As for the geometry, what is the value of the inner radius (cm) ? " ;
    cin >> inner_radius;
    cout << endl;

    cout << "What is the value of the outer radius (cm) ? " ;
    cin >> outer_radius;
    cout << endl;

    cout << "And the value of the cilinder's height (cm)? " ;
    cin >> height;
    cout << endl;

    RNG rng;
    Geometry geometry(inner_radius, outer_radius, height);
    Vector_3 initial_position(geometry.get_inner_radius(),0,0), initial_direction(1,0,0);

    int n_bins;

    cout << "How many bins you want in this simulation? " ;
    cin >> n_bins;
    cout << endl;
    ...
    for(long long int i = 0; i < N_particles; i++){
        ...
        HistoryScore score;
        score = simulate_history(particle, thread_rngs[tid], chosen_material, geometry, chosen_mode, max_collisions, local_counts, bin_width, n_bins);
    ...
    for (size_t j = 0; j < S1.size(); j++){
        ...
        flux[j] = mean[j]/(chosen_material.get_sigma_t()*shell_volume);
        standard_error[j] = std::sqrt(variance[j]/N_particles)/(chosen_material.get_sigma_t()*shell_volume);
    };

    /// the following lines make sense only if Sigma Scattering of the chosen material is equal to 0
   if (chosen_material.get_sigma_s() == 0){
       double theory = std::exp(-chosen_material.get_sigma_a()*(outer_radius - inner_radius));
       cout << "theoretical outer escape (pure absorber) = " << theory << "   difference = " << (mean_escaped_outer - theory) << endl;
    }
